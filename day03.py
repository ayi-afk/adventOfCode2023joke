...
def kinda_convolve2d(list, index, symbols) -> bool | tuple[int, int]:
    i, j = index
    for i, j in [
        (i - 1, j - 1),
        (i - 1, j),
...
def get_through_the_the_stuff(the_stuff: str, empty: str = ".") -> int:
    symbols = get_symbols(the_stuff)
    the_stuff = [list(line) for _, line in enumerate(the_stuff.split("\n"))]
    all_nums = []
    importance = False
    for i, line in enumerate(the_stuff):
        nums = []
        importance = False
        for j, x in enumerate(line):
            if x == empty or x in symbols:
                if importance:
                    all_nums.append(int("".join(nums)))
                    importance = False
                nums = []
                continue
            nums.append(x)
            importance = importance or kinda_convolve2d(the_stuff, (i, j), symbols)
        if not (x == empty or x in symbols) and importance:
            importance = False
            all_nums.append(int("".join(nums)))
...
def get_symbols(lines: str) -> set[str]:
    return {
        char
...
def ohh_noes_get_through_the_the_stuff_again(the_stuff: str, empty=".", symbols=["*"]):
    # finaly could use list def parameter to angry some ppl :D
    the_stuff = [list(line) + ["."] for _, line in enumerate(the_stuff.split("\n"))]
    importance = False
    important_numbers = defaultdict(list)
    for i, line in enumerate(the_stuff):
        nums = []
        importance = False
        cords = 0, 0
        for j, x in enumerate(line):
            if x == empty or x in symbols:
                if importance:
                    important_numbers[cords].append(int("".join(nums)))
                    importance = False
                nums = []
                continue
            nums.append(x)
            current = kinda_convolve2d(the_stuff, (i, j), symbols)
            importance = importance or bool(current)
            cords = current or cords

    return sum(
        reduce(op.mul, (int(v) for v in val), 1)
        for key, val in important_numbers.items()
        if len(val) == 2
    )


test_symbols = get_symbols(test)
data_symbols = get_symbols(data)

print(f"{test_symbols=} \n {data_symbols=}")

print("part 1:", get_through_the_the_stuff(data))
# ^ first one
print("part 2:", ohh_noes_get_through_the_the_stuff_again(data))
